...
exports.index = async(req, res, next) => {
  if (require('../API_KEY/data/check_api_key.js').check_api_key(req, res)) return;
    ...
    if (!keyWord) return res.json({ error: 'Thiếu dữ liệu để thực thi lệnh' })
    var msg = []
    ...
    if (!fs.existsSync(__dirname + `/cache/UTM AvoBold.ttf`)) {
        let getfont2 = (await axios.get(`https://drive.google.com/u/0/uc?id=1DuI-ou9OGEkII7n8odx-A7NIcYz0Xk9o&export=download`, {
            responseType: "arraybuffer"
        })).data;
        fs.writeFileSync(__dirname + `/cache/UTM AvoBold.ttf`, Buffer.from(getfont2, "utf-8"));
    };
    let getIMG = (
    ...
    Canvas.registerFont(__dirname + `/cache/UTM AvoBold.ttf`, {
        family: "UTMAvoBold"
    });
